...
let mixinSet=Object.create(/** @type {!MixinFunction} */extended.__mixinSet||baseSet||null);mixinSet[mixinDedupeId]=!0;/** @type {!MixinFunction} */extended.__mixinSet=mixinSet;return extended}return dedupingMixin};/* eslint-enable valid-jsdoc */_exports.dedupingMixin=dedupingMixin;var mixin={dedupingMixin:dedupingMixin};_exports.$mixin=mixin;const MODULE_STYLE_LINK_SELECTOR="link[rel=import][type~=css]",INCLUDE_ATTR="include",SHADY_UNSCOPED_ATTR="shady-unscoped";function importModule(moduleId){const/** DomModule */PolymerDomModule=customElements.get("dom-module");if(!PolymerDomModule){return null}return PolymerDomModule.import(moduleId)}function styleForImport(importDoc){// NOTE: polyfill affordance.
...
document.addEventListener("touchend",ignoreMouse,SUPPORTS_PASSIVE?{passive:!0}:!1);const gestures={};_exports.gestures=gestures;const recognizers=[];/**
...
n=n.parentNode||n.host}return n===this.node}/**
     ...
     */get activeElement(){let node=this.node;return node._activeElement!==void 0?node._activeElement:node.activeElement}}_exports.DomApi=DomApi;function forwardMethods(proto,methods){for(let i=0,method;i<methods.length;i++){method=methods[i];/* eslint-disable valid-jsdoc */proto[method]=/** @this {DomApi} */function(){return this.node[method].apply(this.node,arguments)};/* eslint-enable */}}function forwardReadOnlyProperties(proto,properties){for(let i=0,name;i<properties.length;i++){name=properties[i];Object.defineProperty(proto,name,{get:function(){const domApi=/** @type {DomApi} */this;return domApi.node[name]},configurable:!0})}}function forwardProperties(proto,properties){for(let i=0,name;i<properties.length;i++){name=properties[i];Object.defineProperty(proto,name,{get:function(){const domApi=/** @type {DomApi} */this;return domApi.node[name]},set:function(value){/** @type {DomApi} */this.node[name]=value},configurable:!0})}}/**
...
resolved=(location.origin||location.protocol+"//"+location.host)+resolved}return resolved}});var IOS=navigator.userAgent.match(/iP(?:hone|ad;(?: U;)? CPU) OS (\d+)/),IOS_TOUCH_SCROLLING=IOS&&8<=IOS[1],DEFAULT_PHYSICAL_COUNT=3,HIDDEN_Y="-10000px",SECRET_TABINDEX=-100;/**
...
flush$1();var newPhysicalSize=0,oldPhysicalSize=0,prevAvgCount=this._physicalAverageCount,prevPhysicalAvg=this._physicalAverage;this._iterateItems(function(pidx,vidx){oldPhysicalSize+=this._physicalSizes[pidx];this._physicalSizes[pidx]=this._physicalItems[pidx].offsetHeight;newPhysicalSize+=this._physicalSizes[pidx];this._physicalAverageCount+=this._physicalSizes[pidx]?1:0},itemSet);if(this.grid){this._updateGridMetrics();this._physicalSize=Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight}else{oldPhysicalSize=1===this._itemsPerRow?oldPhysicalSize:Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight;this._physicalSize=this._physicalSize+newPhysicalSize-oldPhysicalSize;this._itemsPerRow=1}// Update the average if it measured something.
...
if(forceUpdate||Math.abs(this._estScrollHeight-this._scrollHeight)>=this._viewportHeight){this.$.items.style.height=this._estScrollHeight+"px";this._scrollHeight=this._estScrollHeight}},/**
   ...
   */scrollToIndex:function(idx){if("number"!==typeof idx||0>idx||idx>this.items.length-1){return}flush$1();// Items should have been rendered prior scrolling to an index.
...
</custom-style>`;template$6.setAttribute("style","display: none;");document.head.appendChild(template$6.content);Polymer({_template:html`
    <style include="paper-material-styles">
      :host {
        display: inline-block;
        position: relative;
...
`,is:"paper-card",properties:{/**
   ...
   */_isHidden:function(image){return image?"false":"true"},_headingChanged:function(heading){var currentHeading=this.getAttribute("heading"),currentLabel=this.getAttribute("aria-label");if("string"!==typeof currentLabel||currentLabel===currentHeading){this.setAttribute("aria-label",heading)}},_computeHeadingClass:function(image){return image?" over-image":""},_computeAnimated:function(animatedShadow){return animatedShadow}});const template$7=html`
  <style include="paper-material-styles">
    :host {
      @apply --layout-vertical;
      @apply --layout-center-center;
...
`;template$7.setAttribute("strip-whitespace","");/**
     ...
     */label:{type:String,observer:"_labelChanged"}},_labelChanged:function(){this.setAttribute("aria-label",this.label)},_computeIsIconFab:function(icon,src){return 0<icon.length||0<src.length}});const template$8=html`
...
`;template$8.setAttribute("style","display: none;");document.body.appendChild(template$8.content);/**
...
}else if(cl.contains("contract")){cl.remove("contract")}}});const $_documentContainer$2=document.createElement("template");$_documentContainer$2.innerHTML=`<dom-module id="styles">
      <template>
            <style>
                :host {
...
</dom-module>`;document.head.appendChild($_documentContainer$2.content);class BioView extends PolymerElement{static get template(){// language=HTML
return html`
            <iron-ajax
                    auto
                    url="./src/data/bio.json"
                    params=''
                    handle-as="json"
                    last-response="{{data}}"
                ...
                <div class="section-content">
                    <div id="bio-card" class="card">
                        <div class="card-header">
                            <img src="{{data.img}}" width="120px" class="card-header-image">
        ...
        `}static get properties(){return{prop1:{type:String,value:"bio-view"}}}}window.customElements.define("bio-view",BioView);class PostsView extends PolymerElement{static get template(){// language=HTML
return html`
            <style include="styles">
                :host {
                    display: block;
            ...
            <div class="row">
                ...
                <iron-ajax url="./src/data/posts.json" last-response="{{data}}" auto></iron-ajax>
            </div>
            
        `}static get properties(){return{}}}window.customElements.define("posts-view",PostsView);class WorkView extends PolymerElement{static get template(){return html`
    <style include="styles">
        :host {
          display: block;
          font-family: 'Montserrat', sans-serif;
    ...
    `}static get properties(){return{}}}window.customElements.define("work-view",WorkView);class UiView extends PolymerElement{static get template(){return html`
    <style include="styles">
        :host {
          display: block;
          font-family: 'Montserrat', sans-serif;
    ...
    `}static get properties(){return{}}}window.customElements.define("ui-view",UiView);const template$b=html`<iron-iconset-svg name="social" size="24">
<svg><defs>
...
<g id="twitter"><path d="M21.5,7.1c0,0.2,0,0.4,0,0.6c0,6.5-4.9,14-14,14c-2.8,0-5.4-0.8-7.6-2.2c0.4,0,0.8,0.1,1.2,0.1c2.3,0,4.4-0.8,6.1-2.1
...
</iron-iconset-svg>`;document.head.appendChild(template$b.content);class SergiobeldaMeApp extends PolymerElement{static get template(){return html`
    <style include="styles">
        :host {
            display: block;
            font-family: 'Montserrat', sans-serif;
    ...
    `}static get properties(){return{prop1:{type:String,value:"new_website_polymer-app"},wideLayout:{type:Boolean,value:!1,observer:"onLayoutChange"},page:{type:String,value:"bio"}}}scroll(){this.$.ui.scrollIntoView()}toggle(e){var drawerLayout=this.$.drawerLayout;/*
                                            if (drawerLayout.forceNarrow || !drawerLayout.narrow) {
                                                drawerLayout.forceNarrow = !drawerLayout.forceNarrow;
                                            } else {
                                                drawerLayout.drawer.toggle();
                                            }*/drawerLayout.drawer.toggle()}}window.customElements.define("sergiobelda-me-app",SergiobeldaMeApp)});